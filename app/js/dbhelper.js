...
const dbPromise = idb.open('udacity-mws', 2, upgradeDB => {
  switch (upgradeDB.oldVersion) {
    ...
    const reviewStorage = upgradeDB.createObjectStore('reviews', {
      keyPath: 'id'
    });
 ...
 */
  ...
  static idbReadAllOfflineReviews(){
    ...
    .then(allObjs => {
      ...
      // console.log('idbReadAllOfflineReviews()= ', allObjs);
  ...
  /**
   * @description Deletes all entroes from IDB offline-reviews table
   * @param {null}
   * @returns {null} Delets all entries
   * @summary
   *
   */
